...
"spotoroo" stands for spatiotemporal clustering in R of hot spot. It is an algorithm to cluster satellite hot spots, detect ignition points and reconstruct fire movement.
...
Using the built-in dataset `hotspots` as an example. The hot spot data needs to has at least three columns: the longitude, the latitude, and the observed time.

```{r}
str(hotspots)
```

Perform spatiotemporal clustering on this dataset. You need to provide the data, here it is "hotspots", specify which columns correspond to the spatial variables ("lon", "lat"), and which to observed time ("obsTime"). 
...
- "adjDist" sets the maximum intra-cluster spatial distance between hot spots beyond which they would be considered part of a different cluster 
