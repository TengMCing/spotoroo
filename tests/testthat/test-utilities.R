test_that("check_type<_bundle>() works", {
  ...
  expect_error(check_type("numeric", test_obj),
               "Formal argument `test_obj` requires numeric input",
               ignore.case = TRUE)
  ...
  expect_error(check_type_bundle("character", test_obj2, test_obj),
               "Formal argument `test_obj2` requires character input",
               ignore.case = TRUE)

  # logical
  expect_error(check_type("logical", test_obj),
               "Formal argument `test_obj` requires logical input",
               ignore.case = TRUE)
...
test_that("is_length_one<_bundle>() works", {
  ...
  expect_error(is_length_one(test_obj),
               "Formal argument `test_obj` requires input with length equals to 1",
               ignore.case = TRUE)
...
test_that("is_non_negative<_bundle>() works", {
  ...
  expect_error(is_non_negative(test_obj),
               "Formal argument `test_obj` requires non-negative input",
               ignore.case = TRUE)
...
test_that("is_positive<_bundle>() works", {
  ...
  expect_error(is_positive(test_obj),
               "Formal argument `test_obj` requires positive input",
               ignore.case = TRUE)
...
test_that("check_in() works", {
  ...
  expect_error(check_in(c("b", "c"), test_obj),
               'Formal argument `test_obj` only accepts one of these options: "b", "c"',
               ignore.case = TRUE)
...
test_that("is_not_null<_bundle>() works", {
  ...
  expect_error(is_not_null(test_obj),
               'Formal argument `test_obj` requires valid column name',
               ignore.case = TRUE)
...
test_that("equal_length() works", {
  ...
  expect_error(equal_length(test_obj, test_obj2),
               'Formal arguments `test_obj`, `test_obj2` require names of columns with equal lengths',
               ignore.case = TRUE)
...
test_that("check_numeric_column<_bundle>() works", {
  ...
  expect_error(check_numeric_column_bundle(test_obj, test_obj2),
               'Formal argument `test_obj2` requires a name of a numeric column with length greater than 0',
               ignore.case = TRUE)
...
test_that("check_integer_timeID() works", {
  ...
  expect_error(check_integer_timeID(test_obj2),
               'Internal variable timeID is not an integer vector. A proper transformation for the time column is needed. Consider to provide proper values to formal arguments `timeUnit` and `timeStep` to perform the transformation',
               ignore.case = TRUE)
