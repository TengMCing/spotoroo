...
Besides, the **longitude** and **latitude** needs to be in `numeric`.


Using the built-in dataset `hotspots_fin` as an example, it is a `data.frame` with a `lon` (**longitude**) column in `numeric`, a `lat` (**latitude**) column in `numeric` and a `obsTime` (**observed time**) column in `POSIXct`. 

```{r}
str(hotspots_fin)
```

This dataset contains 1070 selected hot spots in Victoria (Australia) during the 2019-2020 Australian bushfire season. More details about this dataset including the data source can be found by using the function `help(hotspots_fin)`
...
A common way to get a better understanding of the hot spot data is to visualize it. This package provides a function `plot_vic_map()` to draw a map of Victoria. It returns a `ggplot` object, so we could add a new layer onto it. Here we use `geom_point()` to draw red dots for hot spots.


From the map, we could find approximately $4$ clusters, but we don't know if they can be further broken down into more clusters. Our goal is to cluster these hot spots into fires in a temporal and spatial manner.
...
If you would like to visualize the hot spots in other areas, you might be able to find the spatial data from the `rnaturalearth` package and make a similar map using the `geom_sf()` function and `ggthemes::theme_map()` function. An example code is given below.
...
**`hotspot_cluster()` is the core function of this package. In most of the cases, you only need to use this function to perform the spatiotemporal clustering algorithm.**  
...
These four arguments controls the clustering process. 
...
In practice, we usually don't have knowledge about the parameters `activeTime` and `adjDist`. And, the number of clusters will generally decrease when you increase these two parameters.
...
Due to the design of the algorithm, the observed time needs to be transformed to discrete time index.  Available time units are "d" (days), "h" (hours), "m" (minutes), "s" (seconds) and "n" (numeric). `timeUnit = "n"` will only be accepted when the observed time is already a vector in `numeric`.

For example, if `timeUnit` is "h" and `timeStep` is $2$, then $1$ time index is $2$ hours. 

## Usage

With specifications of all the arguments, the `hotspot_cluster()` can be used as below. 
...
The function produces messages to tell you some important information of the clustering results such as the number of discrete time indexes, the number of clusters and the proportion of noise. If you would like to silent this function, you could wrap this function with `suppressMessages()` function like the example given below.
...
From the `result`, we know there are $6$ clusters in the clustering results. 
...
The `hotspots` dataset contains information of each hot spot. Particularly, the `membership` column is the membership labels. $-1$ represents noise. 

The `ignition` dataset contains information of each cluster. Similarly, the `membership` column is the membership labels. And the `lon` and `lat` are the coordinate information of the ignition points. 
...
You could choose to extract all clusters along with noise points by setting `cluster = "all"` and `noise = TRUE`.

```{r eval = FALSE}
extract_fire(result, cluster = "all", noise = TRUE)
```

Or only a subset of clusters without any noise by providing a vector of membership labels to the argument `cluster` and set `noise = FALSE`.
...
In principal, the parameters `activeTime` and `adjDist` are determined by the professional knowledge of fire behaviour, but in practice, we generally don't know much about them.

In the rest of the section, we will show one of the methods to choose proper values for these two parameters.

We first set `minPts = 4` and `minTime = 3`. You could set different values for `minPts` and `minTime`.

We then could do a grid search for `activeTime` and `adjDist` in a sensible range. Here, we set `adjDist` $\in$ `[500,1000,1500,2000,2500,3000,3500,4000]` and `activeTime` $\in$ `[6,12,18,24,30,36,42,48]`. For each pair of `activeTime` and `adjDist`, we record its proportion of noise as the metric for comparison.

The following code records the proportion of noise. It may takes around 10 minutes to run. You could have a try if you like. 
...
It works like the [scree plot](https://en.wikipedia.org/wiki/Scree_plot) used in principal component analysis.
...
You could produce a plot of the clustering results. There are three types of plots, which are "def" (default), "mov" (fire movement) and "timeline" (timeline). T
